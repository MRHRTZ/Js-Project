...
const fs = require("fs");
let bg = JSON.parse(fs.readFileSync('./unsplash.json'))
...
let acakbg = Math.floor(Math.random() * bg.length - 1);
     // (async () => {
          // text = ['Goodbye:(','RIP','Berduka','Keluar Hemm:(']
     //      const image = await new Canvas.Goodbye()
     //           .setText('title', text[Math.floor(Math.random() * text.length - 1)])
     //           .setUsername("xixi52")
     //           .setDiscriminator("0001")
     //           .setMemberCount("42")
     //           .setGuildName("Server DEV")
     //           .setAvatar("https://images.unsplash.com/photo-1609650793543-74a707970d29?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=325&q=80")
     ...
     //           .toAttachment();
     //      fs.writeFileSync('./xixi.png', image.toBuffer())
     // })()
     (async () => {
